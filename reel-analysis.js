...
const reelHeroCard = $('reelHeroCard');
const kpiGrid = $('kpiGrid');
const behaviorGrid = $('behaviorGrid');
const timelineChart = $('timelineChart');
const timelineMeta = $('timelineMeta');
const hourChart = $('hourChart');
const weekdayChart = $('weekdayChart');
const feelingChart = $('feelingChart');
const intentGrid = $('intentGrid');
const writtenGrid = $('writtenGrid');
...
function handleReelsUpdate() {
    ...
    noReelsState.hidden = true;

    const urlReel = (new URLSearchParams(window.location.search).get('reel') || '').trim().toUpperCase();

    if (!activeReelId) {
        activeReelId = allReels.find((r) => r.id === urlReel) ? urlReel : allReels[0].id;
    } else if (!allReels.find((r) => r.id === activeReelId)) {
        activeReelId = allReels[0].id;
    }

    reelSelect.innerHTML = allReels.map((r) => {
        const label = r.id + ' — ' + (r.title || 'Untitled');
        const sel = r.id === activeReelId ? ' selected' : '';
        return `<option value="${escapeAttr(r.id)}"${sel}>${escapeHTML(truncate(label, 70))}</option>`;
    }).join('');
    ...
    const kpis = [
        {
            ...
            sub: stats.total < 3 ? 'Sample too small'
                : stats.total < RECOMMENDATION_THRESHOLD ? 'Building sample'
                : 'Sufficient sample',
        ...
        },
        {
            ...
            sub: ratingLabel(stats.avg),
            icon: '<path d="m12 2 3 6.5 7 1-5 4.9 1.2 7L12 18l-6.2 3.4L7 14.4 2 9.5l7-1z"/>'
        },
        {
            label: 'Recommend Rate',
            value: stats.recommendPct + '%',
            sub: 'Want more like this',
            icon: '<path d="M14 9V5a3 3 0 00-3-3l-4 9v11h11.28a2 2 0 002-1.7l1.38-9a2 2 0 00-2-2.3zM7 22H4a2 2 0 01-2-2v-7a2 2 0 012-2h3"/>'
        },
        {
            label: 'Repeat Intent',
            value: stats.repeatPct + '%',
            sub: 'Likely to engage again',
            icon: '<path d="M21 12a9 9 0 11-6.219-8.56"/><path d="M22 4l-10 10"/>'
        }
    ];
    ...
    const cards = [
        {
            ...
            value: stats.firstDelayMs ? formatRelative(stats.firstDelayMs) : '—',
            sub: 'From publish to first response',
            icon: '<circle cx="12" cy="12" r="10"/><path d="M12 6v6l4 2"/>'
        },
        {
            label: 'Response Velocity',
            value: stats.velocity.toFixed(1) + ' / day',
            sub: 'Average daily responses',
            icon: '<path d="M13 2L3 14h9l-1 8 10-12h-9z"/>'
        },
        {
            label: 'Active Window',
            value: stats.spanDays + ' day' + (stats.spanDays === 1 ? '' : 's'),
            sub: 'First to last response',
            icon: '<path d="M3 3v18h18"/><path d="M18.7 8l-5.1 5.2-2.8-2.7L7 14.3"/>'
        },
        {
            label: 'Peak Time',
            value: stats.peakHourLabel || '—',
            sub: 'Highest activity window (IST)',
            icon: '<circle cx="12" cy="12" r="10"/><path d="M12 6v6l4 2"/>'
        },
        {
            label: 'Peak Weekday',
            value: stats.peakWeekday ? stats.peakWeekday[0] : '—',
            sub: 'Most responses on this day',
            icon: '<rect x="3" y="4" width="18" height="18" rx="2"/><path d="M16 2v4M8 2v4M3 10h18"/>'
        },
        {
            label: 'Written Rate',
            value: stats.writtenPct + '%',
            sub: 'Responses with a message',
            icon: '<path d="M21 15a2 2 0 01-2 2H7l-4 4V5a2 2 0 012-2h14a2 2 0 012 2z"/>'
        }
    ];

    behaviorGrid.innerHTML = cards.map((c) => `
        <div class="exec-card">
            <div class="exec-icon">
...
function renderTimeline(stats) {
    ...
    if (!keys.length) {
        timelineChart.innerHTML = chartEmpty('No timeline data');
    ...
    timelineMeta.textContent = keys.length + ' day' + (keys.length === 1 ? '' : 's');

    timelineChart.innerHTML = `<div class="dist-list">${trimmed.map(({ k, v }) => {
...
function renderHourChart(stats) {
    ...
    hourChart.innerHTML = `<div class="dist-list">${stats.hourBuckets.map((v, i) => {
        const width = Math.round((v / max) * 100);
...
function renderWeekdayChart(stats) {
    ...
    weekdayChart.innerHTML = `<div class="dist-list">${values.map(({ k, v }) => {
        const width = Math.round((v / max) * 100);
        const isPeak = v === peakVal && v > 0;
        return `
            <div class="dist-bar">
...
function renderFeeling(stats) {
    ...
    feelingChart.innerHTML = `<div class="dist-list">${stats.feelings.map(([k, v]) => {
        const width = Math.round((v / max) * 100);
        const isPeak = v === max;
        return `
            <div class="dist-bar">
...
function renderIntent(stats) {
    ...
    cards.push({
        label: 'Would Watch More',
        value: pct(moreYes, stats.total) + '%',
        sub: moreYes + ' of ' + stats.total + ' want similar content',
        tone: 'positive'
    });
    ...
    cards.push({
        label: 'Repeat Intent',
        value: stats.repeatPct + '%',
        sub: veryLikely + ' very likely · ' + likely + ' likely',
        tone: stats.repeatPct >= 50 ? 'positive' : (stats.repeatPct >= 25 ? 'neutral' : 'negative')
    });

    if (stats.wants.length) {
        const [w, c] = stats.wants[0];
        cards.push({
            label: 'Top Content Request',
            value: truncate(w, 24),
            sub: pct(c, stats.total) + '% requested this',
            tone: 'neutral'
        });
    }
    ...
    if (stats.topFeeling) {
        ...
        cards.push({
            ...
            value: truncate(f, 20),
            sub: pct(c, stats.total) + '% selected this',
            tone: 'positive'
        });
    }

    intentGrid.innerHTML = cards.map((c) => `
...
function renderWritten(stats) {
    ...
    if (!stats.topWords.length) {
        writtenThemes.innerHTML = chartEmpty('Not enough written feedback to detect themes');
        return;
    }

    writtenThemes.innerHTML = `<div class="ask-grid">${stats.topWords.map(([w, c]) => `
...
function renderRecommendation(stats) {
    if (stats.total < RECOMMENDATION_THRESHOLD) {
        recommendationBlock.innerHTML = lockedHTML(stats);
        return;
    }
        ...
        ${working.length ? `
            <div class="action-center" style="margin-top:14px;">
                ...
                <div class="alert-list">
                    ${working.map((t) => `
                        <div class="alert-item alert-item--positive">
                            <div class="alert-desc">${escapeHTML(t)}</div>
                        </div>
                    `).join('')}
                </div>
            </div>
...
function computeVerdict(stats) {
    ...
    if (avg >= 4.5 && repeatPct >= 60) {
        return {
            cls: 'high',
            label: 'Top Performer',
            summary: `This reel is a strong asset for Rudra Bhakti. High ratings and high repeat intent across ${total} responses. Continue the format.`
        };
    }
    if (avg >= 4 && repeatPct >= 40) {
        return {
            cls: 'strong',
            label: 'Strong Performer',
            summary: `Consistent positive reception across ${total} responses with healthy intent to return. Consider minor variations to test longevity.`
        };
    }
    if (polarizing) {
        return {
            cls: 'polarizing',
            label: 'Polarizing',
            summary: `Audience reaction is mixed. Average is ${avg.toFixed(1)}/5 but variance is high — different segments respond very differently. Investigate low ratings.`
        };
    }
    if (avg >= 3.5) {
        return {
            cls: 'stable',
            label: 'Stable',
            summary: `Solid baseline performance at ${avg.toFixed(1)}/5. Neither winning nor underperforming. Use as a reference, not a template.`
        };
    }
    return {
        cls: 'under',
...
function buildWorkingList(stats) {
    ...
    if (avg >= 4.5) list.push(`Audience satisfaction is excellent at ${avg.toFixed(1)}/5 across ${total} responses.`);
    else if (avg >= 4) list.push(`Strong rating of ${avg.toFixed(1)}/5 indicates this format connects well.`);

    if (topFeeling) {
        const p = pct(topFeeling[1], total);
        if (p >= 30) list.push(`"${topFeeling[0]}" dominates emotional response (${p}%), showing a clear, consistent feeling.`);
    }
    if (recommendPct >= 60) list.push(`${recommendPct}% of audience wants more content like this.`);
    if (repeatPct >= 60) list.push(`${repeatPct}% said they are likely to engage with similar reels again.`);
    if (writtenPct >= 40) list.push(`High written engagement (${writtenPct}%) — audience is invested enough to type a response.`);
    if (peakHourLabel) list.push(`Peak response window is ${peakHourLabel} IST${peakWeekday ? ' on ' + peakWeekday[0] : ''}.`);
...
function buildWatchList(stats) {
    ...
    if (avg >= 4 && repeatPct < 40) list.push(`Ratings are high but only ${repeatPct}% are likely to return. Satisfaction is not converting into repeat intent.`);
    if (avg < 3.5 && total >= 10) list.push(`Average rating of ${avg.toFixed(1)}/5 is below the 3.5 threshold.`);
    if (stdDev >= 1.2) list.push(`Rating variance is high (σ = ${stdDev.toFixed(2)}). Audience is split — investigate which segments rated low.`);
    if (recommendPct < 40 && avg >= 3.5) list.push(`Only ${recommendPct}% want more of this. Consider what is missing for the remaining audience.`);
    if (!stats.topWant && total >= 10) list.push(`No clear content preference was captured. Consider a more targeted "what next" question.`);
...
function buildActionList(stats) {
    ...
    if (avg >= 4.5 && repeatPct >= 60) list.push(`Replicate this format. Continue the same visual style and emotional tone in the next 2–3 reels.`);
    if (peakHourLabel) list.push(`Schedule future similar reels to publish near ${peakHourLabel} IST${peakWeekday ? ' (' + peakWeekday[0] + ')' : ''}.`);
    if (topWant && stats.wants.length) list.push(`Test a new reel themed "${topWant[0]}" — requested by ${pct(topWant[1], stats.total)}% of the audience.`);
    if (avg >= 4 && repeatPct < 40) list.push(`Add stronger retention hooks (opening line, question, or cliffhanger) to convert satisfaction into repeat intent.`);
    if (avg < 3.5) list.push(`Review written feedback for this reel and identify the top 2 improvement requests before producing similar content.`);
    if (recommendPct >= 50 && !list.some((t) => t.startsWith('Test a new reel themed'))) list.push(`Continue this content direction with minor variations over the next 3 reels to confirm the pattern.`);
    if (!list.length) list.push(`Continue collecting responses and revisit after the next batch of feedback.`);
